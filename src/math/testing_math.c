...
/*   Updated: 2026/01/19 16:31:03 by vramacha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//its how far you would travel in a straight line if u were to walk from one end
...
// addint two tuples
//imagin that u have a point (3,-2,5,1) and a vector (-2, 3,1,0) and u want to
// know ehre you woult be if u followed thr vector from that point the answear
// comes via adding - adding the two tuples toghether
// void	add_tuples(double *new_tuple, double *tuple1, double *tuple2)
// {
// 	int	i = 0;
// 	// dobra czyli mamy jakby dwa ciagi liczb x y z i jesli bedziemy sie trzymac
...
// substracting tuplets is usefull when it comes to light and shading, when you
// need to find the vector that points to your light source
...
//how far u would travel in straight line if u were to walk from one end of
// vector to the other
double	magnitude(t_tup a);

// void		normalize(t_tup res, t_tup a);

// vectors with magnitude of 1 are called a unit vectors and u will use them
// when computing the MATRIX when determinin the direction prepend
int	main(void)
{
	t_tup	tup;
	t_tup	tmp;

	init_vector(tup, 1, 2, 2);
	printf("sqrt: %g\n", sqrt(tup[0] * tup[0] + tup[1] * tup[1] + tup[2] * tup[2]
			+ tup[3] * tup[3]));
	printf("dot-prod: %g\n", dot_prod(tup, tup));
	...
	printf("transpose: ((%g,%g,%g,%g),(%g,%g,%g,%g),\
		(%g,%g,%g,%g),(%g,%g,%g,%g))\n",
		res[0][0], res[0][1], res[0][2], res[0][3],
		res[1][0], res[1][1], res[1][2], res[1][3],
		res[2][0], res[2][1], res[2][2], res[2][3],
	...
	rotation_mat(r, d);
	printf("rot-mat: ((%g,%g,%g,%g),(%g,%g,%g,%g),\
		(%g,%g,%g,%g),(%g,%g,%g,%g))\n",
